...
class ContractRepository:
    ...
    def read(self):
        ...
        contracts = [Contract(row['name'], row['phone'], row['email'], row['address'], row['vehicle_id'], row['vehicle_status'], row['employee_id'], row['location'], row['date_from'], row['date_to'], row['loan_date'], row['loan_status'], row['return_date'], row['total'], row['id']) for row in rows]
        return contracts
