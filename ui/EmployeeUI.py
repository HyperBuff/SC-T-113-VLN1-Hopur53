...
class EmployeeUI:
    ...
    def print(self, employees, start, end, current_page, last_page):
        if len(employees) > 0:
            print("|{:^6}|{:^15}|{:^20}|{:^25}|{:^14}|{:^20}|{:^26}|".format("ID", "Role", "Name", "Email", "SSN", "Mobile phone", "Location"))
            print('-' * 134)
            for i in range(start, end):
                print("|{:^6}|{:<15}|{:<20}|{:<25}|{:<14}|{:<20}|{:<26}|".format(employees[i].id, employees[i].role, employees[i].name, employees[i].email, employees[i].ssn, employees[i].phone, self.logic.get_location_by_id(employees[i].location_id).__str__()))
            print("{:^134}".format("Page {} of {}".format(current_page, last_page)))
            self.printer.new_line()
        else:
