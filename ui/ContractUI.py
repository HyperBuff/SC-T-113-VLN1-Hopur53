...
class ContractUI:
    ...
    def print(self, contracts, start, end, current_page, last_page):
        if len(contracts) > 0:
            print("|{:^4}|{:^40}|{:^27}|{:^27}|{:^15}|{:^15}|{:^15}|{:^20}|{:^15}|{:^15}|".format("ID", "Customer", "Vehicle", "Location", "Date from", "Date to", "Contract Date", "Contract status", "Pickup date", "Dropoff date"))
            print('-' * 204)
            for i in range(start, end):
                print("|{:^4}|{:^40}|{:^27}|{:^27}|{:^15}|{:^15}|{:^15}|{:^20}|{:^15}|{:^15}|".format(contracts[i].id, self.logic.get_customer_by_id(contracts[i].customer_id).__str__(), self.logic.get_vehicle_by_id(contracts[i].vehicle_id).__str__(), self.logic.get_location_by_id(contracts[i].location_id).__str__(), contracts[i].date_from, contracts[i].date_to, contracts[i].contract_date, contracts[i].contract_status, contracts[i].pickup_date, contracts[i].dropoff_date))
            print("{:^204}".format("Page {} of {}".format(current_page, last_page)))
            self.printer.new_line()
        else:
